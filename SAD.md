...
**Important Note:** The original Python script functions as a **command-line tool** and should remain **unchanged** in terms of **API calls and prompt structure**. Any modifications should be strictly limited to integrating the existing script into a broader system without altering its core logic.
...
2. **PDF Processing & AI Transcription**
   ...
   - Images are sent to **GPT-4o** via API for transcription **(API calls and prompts remain untouched).**
...
This architecture balances **scalability, security, and ease of use**, leveraging **Google Cloud** services for reliability. The system is built to handle **large-scale document processing** while ensuring the **original Python script remains unchanged** in its **command-line execution, API calls, and prompts**.
```
